import { display, type NdaFormData } from "./nda";

/** Build a filled Mutual NDA cover page + standard terms from form values (Common Paper CC BY 4.0). */
export function buildFilledNdaMarkdown(data: NdaFormData): string {
  const mndaTerm =
    data.mndaTermMode === "expires"
      ? `Expires ${display(data.mndaTermYears, "1")} year(s) from Effective Date.`
      : "Continues until terminated in accordance with the terms of the MNDA.";

  const confidentiality =
    data.confidentialityMode === "years"
      ? `${display(data.confidentialityYears, "1")} year(s) from Effective Date, but in the case of trade secrets until Confidential Information is no longer considered a trade secret under applicable laws.`
      : "In perpetuity.";

  const mods = data.modifications.trim() || "None.";
...
Governing Law: ${display(data.governingLaw, "[Fill in state]")}

Jurisdiction: ${display(data.jurisdiction, "[Fill in city or county and state]")}
...
| | Party 1 | Party 2 |
| --- | --- | --- |
| Signature | ${display(p1.signature)} | ${display(p2.signature)} |
| Print Name | ${display(p1.printName)} | ${display(p2.printName)} |
| Title | ${display(p1.title)} | ${display(p2.title)} |
| Company | ${display(p1.company)} | ${display(p2.company)} |
| Notice Address | ${display(p1.noticeAddress)} | ${display(p2.noticeAddress)} |
| Date | ${display(p1.date)} | ${display(p2.date)} |
